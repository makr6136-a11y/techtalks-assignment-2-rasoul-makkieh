...
The stack decision itself is settled by seniority, not preference. The non-local group (listings 4, 5, 6) has no genuine entry point — 5+ years, four years behind a "Junior" title, and another 5+ year Lead I role — and places greater emphasis on deployment, cloud and engineering process. Six months does not close that gap, and pretending otherwise would make this roadmap fiction. The local group contains the one reachable role in the sample: listing 2, an intern position asking for basic-to-intermediate PHP, JavaScript, HTML, CSS, Git, APIs and databases. That listing is effectively the specification for months 1–6, and I already hold two of its requirements outright.

So the sequence is: shared foundation first (months 1–3), local stack second (months 4–5), shipped evidence third (month 6). Infrastructure — CI/CD, cloud and IaC requirements concentrated in experienced and non-local roles — is deliberately out of scope here. It is the bridge from the local market to broader opportunities, and it is the right investment for months 7–18, once there is a job to build it on top of.
...
|-------|-------|----------------|-----------|
| 1 | JavaScript (ES6+) and Git | JavaScript is 4/6 and named explicitly in listing 2 as "ES6+". Git is explicitly required in 4/6 listings, including listing 6. It is also the fastest way to convert existing HTML/CSS into something interactive | I can write ES6 (fetch, promises/async, destructuring, modules) without a tutorial open, and have a repo with a branch-and-merge history rather than a single main branch |
...
| 3 | SQL depth and schema design | 6/6 under the stated counting method, and listings 1 and 4 ask for schema design specifically — not just querying. My current SQL is basic, so this is upgrading a held skill rather than starting one. Overlaps Database Programming almost entirely | I have designed a normalized multi-table schema from a written brief, and can explain an index choice and a slow-query fix out loud |
...
| 5 | Authentication and security | 3/6, and required even of the role labelled Junior — listing 5 expects vulnerability remediation and dependency patching, listing 3 names permissions and data privacy, and listing 4 requires secure applications. It is the requirement least likely to be forgiven in an interview | My month-4 app has registration, hashed passwords, sessions and role-based access, and I can explain SQL injection, XSS and BOLA with the fix for each |
| 6 | Deploy, document, apply | Documentation appears in 4/6 (runbooks and SOPs in 1, technical and release docs in 3, root-cause write-ups in 5, documented code in 6), and some non-technical requirement appears in all six. Listing 1 names written English outright. A project nobody can visit is not evidence | The project is live at a URL, has a README and API documentation written in English, sits in a public repo with a readable commit history, and I have submitted applications to intern and junior local roles |

## What I am deliberately not learning

**Java and Spring — Java 1/6, Spring 0/6.** Java appears once as an alternative backend language in listing 6, while Spring is absent. OOP II continues as coursework because the concepts transfer, but the sample does not justify making Java the main six-month focus.

**C++ — 0/6.** Does not appear anywhere in the sample, not even as a nice-to-have.

**Next.js and React — 1/6 each.** Both appear only in listing 4, a remote role requiring five years. Deferred to months 7+, after a local role, rather than dropped — the JavaScript work in month 1 is the prerequisite either way, so nothing is wasted.

**TypeScript — 1/6.** Listing 4 alone. Worth adding once JavaScript is solid; worthless before then.

**GraphQL — 1/6.** Listing 4 only, and it asks for GraphQL alongside REST rather than instead of it. REST is the requirement; GraphQL is the extra.

**CI/CD, cloud, Docker and Infrastructure as Code — 2/6 required, 3/6, 1/6, 1/6.** CI/CD is also preferred in listing 6. These tools are concentrated in experienced roles and form part of the bridge from a local entry-level profile to broader opportunities, which is why they belong after a first job rather than before one.

**NoSQL — 2/6.** Listings 4 and 6, both alongside relational database options rather than replacing them. Relational SQL remains the priority for month 3.
...
**Testing frameworks — required in listing 6.** Jasmine/Karma and Pytest are required there, while Jest, Cypress and TDD are preferred in listing 4. Automated testing remains deferred because the reachable intern role asks only for debugging and basic testing.
